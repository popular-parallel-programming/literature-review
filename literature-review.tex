...
\begin{document}
\Ttitle{Declarative Parallel Programming in Spreadsheet End-User Development}
\Tsubtitle{A Literature Review}
\Tauthors{Florian Biermann}

\Tyear{2016}
\Tmonth{February}
\Ttrnumber{192}
\TISBNno{978-87-7949-357-5}
...
\begin{center}
  \begin{tabular}{ccc}
    \texttt{map g (map f xs)} & $\Longrightarrow$ & \texttt{map (f . g) xs}
  \end{tabular}
\end{center}
...
\begin{figure}
  \begin{center}
    \begin{tabular}{|g|c|c|c|}
      ...
      \rowcolor{lightgray}
        & A  & B                & C                \\ \hline
      1 & 23 & \texttt{= F(A1)} & \texttt{= G(B1)} \\ \hline
      2 & 42 & \texttt{= F(A2)} & \texttt{= G(B2)} \\ \hline
      3 & 96 & \texttt{= F(A3)} & \texttt{= G(B3)} \\ \hline
    \end{tabular}
    ~
    \begin{tabular}{|g|c|c|c|}
      \hline
      \rowcolor{lightgray}
        & A  & B                & C                    \\ \hline
      1 & 23 & \texttt{= F(A1)} & \texttt{= J(B1, B2)} \\ \hline
      2 & 42 & \texttt{= F(A2)} & \texttt{= J(B2, B3)} \\ \hline
      3 & 96 & \texttt{= F(A3)} & \texttt{= J(B3, B1)} \\ \hline
    \end{tabular}
  \end{center}
  \caption{Examples of non-fusable stencil-like structures on interpreted spreadsheets. \textit{Left:} columns~\textit{B} and \textit{C} cannot be fused because every cell needs to evaluate to the result of its formula. \textit{Right:} columns~\textit{B} and \textit{C} cannot be fused because the intermediate values of column \textit{B} are referenced more than once in column~\textit{C}.}
...
Destructive array updates~\cite{Sastry:1994:PDU:182409.182486} are also only interesting in the context of single cells. The analysis is much simpler, for the same reason of why fusion across cells is not possible and because there are no let-bindings within the formula of a single cell.
