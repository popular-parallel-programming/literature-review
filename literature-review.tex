...
splitting~\cite{Bergstrom:2010:LTS:1863543.1863558}. Work is
distributed by a master thread that communicates with other threads
via a work-stealing queue like technique. Arrays are represented via
binary trees of arrays. For each portion of work that the master
thread performs, it checks if any of the remaining threads are
idle. If so, it splits the remaining array in two, dispatches the
latter half to the idle threads and continues to the next portion of
work. This heuristic shows low overhead in experimental settings
...
\cite{Hwang:1995:AOS:209936.209949,
  ...
  Lippmeier:2012:GPA:2364506.2364511}\todo{Write about these
  publications.}
